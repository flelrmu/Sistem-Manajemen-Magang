...
const fileFilter = (req, file, cb) => {
  ...
  if (fileTypes.includes(file.mimetype)) {
    ...
    cb(
      new Error(
        `Tipe file tidak diizinkan. Tipe yang diizinkan: ${fileTypes.join(
          ", "
        )}`
      ),
      false
...
const uploadMiddleware = {
  ...
  deleteOldFile: async (req, res, next) => {
    try {
      ...
      switch (resource) {
        ...
        case "reports":
          query =
            "SELECT file_path FROM laporan WHERE mahasiswa_id = ? ORDER BY created_at DESC LIMIT 1";
          folder = "reports";
          break;
        case "logbooks":
          query =
            "SELECT file_dokumentasi FROM logbook WHERE mahasiswa_id = ? ORDER BY created_at DESC LIMIT 1";
          folder = "logbooks";
          break;
        case "permissions":
          query =
            "SELECT file_bukti FROM izin WHERE mahasiswa_id = ? ORDER BY created_at DESC LIMIT 1";
          folder = "permissions";
          break;
        default:
          return next();
  ...
  createUploadDirs: (req, res, next) => {
    const dirs = [
      ...
      "uploads/logbooks",
      "uploads/permissions",
      "uploads/paraf", // Add paraf directory
    ];
